...
package object phases {
  ...
  @compileTimeOnly("io.github.jpivarski.phases.declare is intended as a static (compile-time) annotation: include org.scalamacros.paradise in your build!")
  class declare extends StaticAnnotation {
    def macroTransform(annottees: Any*): Any = macro declare_impl
  }

  def declare_impl(c: whitebox.Context)(annottees: c.Expr[Any]*): c.Expr[Any] = {
    import c.universe._
    ...
    val companionObjectDef = companionPair.collect({case x: ModuleDef => x}).
      headOption.getOrElse(q"object ${TermName(className.toString)}")
    ...
    def makeTransitionMethod(from: String, to: String): Tree = {
      ...
      DefDef(
        Modifiers(),
        TermName("to" + to.head.toUpper + to.tail),
        List(),
        List(methodArgs),
        TypeTree(),
        Apply(Select(New(Select(Ident(TermName(className.toString)), TypeName(to))), termNames.CONSTRUCTOR), constructorArgs))
    }
    ...
    class ClassDefTransformer(phaseToKeep: Option[String]) extends Transformer {
      ...
      def transformInit(tree: Tree, args: List[String]): Tree = tree match {
        case Block(Apply(fun, _) :: Nil, expr) => transform(Block(Apply(fun, args map {x => Ident(TermName(x))}) :: Nil, expr))
        ...
        case ClassDef(mods, name, tparams, Template(parents, self, body)) =>
          ...
          ClassDef(
            ...
            phaseToKeep match {
              ...
              case Some(phase) => TypeName(phase)  // the subclasses should be named after their phase
            },
